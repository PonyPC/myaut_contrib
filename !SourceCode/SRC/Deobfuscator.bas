Attribute VB_Name = "Deobfuscator"
Option Explicit
Option Compare Text
...
Public Sub init()
 ...
 '=== Open & Test Obfu-Type ===
  ...
  'Convert to Accii if it's unicode
   ...
   Dim data As String
   data = fso.ReadFile(FILE.filename)
   
   If IsUTF16File Then
      NewScript = StrConv(data, vbFromUnicode, LocaleID)
   Else
      If IsUTF8File Then
         NewScript = DecodeUTF8(data)
      Else
         FILE.Position = 0
         NewScript = data
      End If
...
'   DeobfuRounds = 0
   Do
      ...
      If lastScript <> NewScript Then
            Log "Deobfucation succeed."
            SaveScriptData NewScript, True 'IsTidyed
      Else
            Log "Deobfucation produced no changes, exiting loop"
            If IsTidyed = False Then RunTidy NewScript
            Err.Raise ERR_NO_OBFUSCATE_AUT
            Exit Sub
      End If
   ...
   Dim TBLFileData As New StringReader
    ...
    Do
         If Err Then
             ...
             Else
                 Log ">>>> Can't open TBL-file " & Err.Description
                 Exit Do
             End If
         End If
       
       On Error Resume Next
       TBLFileData.data = fso.ReadFile(StringTBLFileName.filename)
    Loop While Err
...
' === Check TBLStringSeperator ===
   ...
   Else
      If Not isAutomationRun Then
            ...
            Loop
      Else
            Log ">>>> DeObfuscate_VanZande1_0_15 could not auto determine TBLStringSeperator run in manual mode to specify!"
      End If
...
'         SearchString = MatchesSplitted(i + 1)
        ...
        'filter out names where seach&replace was already done
         If Not KeyExistsInCollection(c, "str:" & SearchString) Then
            c.Add "", "str:" & SearchString
